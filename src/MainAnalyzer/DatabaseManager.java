...
public class DatabaseManager {
    ...
    public DatabaseManager(String _URL, String _USER, String _DB_Main, String _DB_Module, String _PASSWORD) {
        this.__URL = _URL;
        this.__USER = _USER;
    ...
    public void updateTables(boolean db_main, boolean db_module) {
        ...
        try {
            ...
            try {
                ...
                // Добавляем вручную нужную запись
                long manual_id = 10000000L;
                String manual_name = "Внешние источники";
                Timestamp manual_start = Timestamp.valueOf("1991-01-01 00:00:00");
                Date manual_end = null;

                rowOs.add(new rowOrders(manual_id, manual_name, manual_start, manual_end));
        ...
        // Дополнительный блок после основного запроса
        query = "SELECT id FROM public.bs_goods WHERE id >= 10000000";
        ...
        try {
            ...
            try {
                while (resultSet.next()) {
                    long g_id = resultSet.getLong("id");

                    // Добавляем строку в rowmesGOs как псевдозаказ
                    rowmesGOs.add(new rowWorkOrder(
                            g_id,              // id_goods
                            10000000L,         // id_order фиксированный
                            0.0,               // nqtybase
                            null,              // dplanbegin
                            null               // dplanend
                    ));
...
// Запрос для получения данных из prs_etrep
        query = "SELECT DISTINCT bg.id as id_g, bc.id as id_c, pe.ndeldays as n_time, "
                + "pe.nqty as n_qty, pe.nprc as nprc, pe.nqtybase as n_qty_b, pe.nprcbase as nprc_b "
                + "FROM public.prs_etrep pe "
                + "JOIN bs_goods bg on pe.gidgdssrv = bg.gid "
                + "JOIN bs_contras bc on pe.gidsettler = bc.gid";
        ...
        try {
            ...
            try {
                while(resultSet.next()) {
                    ...
                    double g_qual = 1.0;  // Хорошее качество товара
                    double c_del = 0.0;   // Задержка контрагента (по умолчанию 0.0)
                    rowprsCGs.add(new rowLot(g_id, id_g, n_time, n_qty, nprc, n_qty_b, nprc_b, g_qual, c_del));
                }
        ...
        // Новый запрос для получения данных из supplier_summary
        query = "SELECT DISTINCT "
                + "    bc.id AS id_c, "
                + "    ss.contract_duration_days AS ndeliverytime, "
                + "    po.ktru_okpd2_codes AS ktru_okpd2, "
                + "    REPLACE(bc.scaption, E'\\n', ' ') AS name, "
                + "    bg.id AS id_goods, "
                + "    0.0 AS nqty, "
                + "    0.0 AS nprc, "
                + "    0.0 AS nqtysale, "
                ...
                + "JOIN public.bs_contras bc ON bc.sinn = ss.inn "
                + "JOIN public.procurement_objects po ON po.ktru_okpd2_codes = ss.okpd2_codes "
                + "JOIN public.bs_goods bg ON bg.okpd2 = po.ktru_okpd2_codes "
                + "WHERE ss.contract_id IS NOT NULL "
                + "  AND bc.id >= 10000000";

        try {
            ResultSet resultSet2 = this.executeQuery(db_module, query);
            ...
            try {
                while(resultSet2.next()) {
                    ...
                    rowprsCGs.add(new rowLot(id_goods, id_contras, ndeliverytime, n_qty, nprc, n_qty_b, nprc_b, g_qual, c_del));
                }
            } catch (Throwable var62) {
                if (resultSet2 != null) {
                    try {
...
// Вставка данных в prs_lot
        query = "INSERT INTO public.prs_lot (id_goods, id_contras, ndeliverytime, nqty, nprc, nqtysale, nprcsale, ngoodquality, ncontrasdelay) "
                + "VALUES (?,?,?,?,?,?,?,?,?)";
        ...
        query = "UPDATE public.prs_lot pl " +
                ...
                "  AND pl.id_contras < 10000000 " +  // Только внутренние контрагенты
                "  AND pl.id_goods < 10000000";
    ...
    public void updateCGsUserCritValues(boolean db_module) {
        String query = "INSERT INTO public.module_lotcriterion (id_contras, id_goods, ndeliverytime) " +
                "SELECT DISTINCT bc.id AS id_contras, bg.id AS id_goods, pl.ndeliverytime " +
                "FROM bs_order bo " +
                "JOIN mes_workorder mw ON bo.id = mw.id_order " +
